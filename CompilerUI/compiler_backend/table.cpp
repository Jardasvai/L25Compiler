...
void Table::debugTable(int start)
{
    ...
    if (start >= tx)
    {
        ...
        fas_stream << "Empty table\n";
        ftable_stream << "Empty table\n";
        return;
    }
    ...
    for (int i = start; i < tx; i++)
    {
        ...
        case Objekt::string:
            ...
            fas_stream << i << " " << item.name << " string level:" << item.level << " addr:" << item.adr << " value:\"" << item.str_val << "\"\n";
            ftable_stream << i << " " << item.name << " string level:" << item.level << " addr:" << item.adr << " value:\"" << item.str_val << "\"\n";
            break;


        case Objekt::variable:
            std::cout << i << " " << item.name << " variable level:" << item.level << " addr:" << item.adr << "\n";
            fas_stream << i << " " << item.name << " variable level:" << item.level << " addr:" << item.adr << "\n";
            ftable_stream << i << " " << item.name << " variable level:" << item.level << " addr:" << item.adr << "\n";
            break;

        case Objekt::int_pointer:
            std::cout << i << " " << item.name << " int_pointer level:" << item.level << " addr:" << item.adr << "\n";
            fas_stream << i << " " << item.name << " int_pointer level:" << item.level << " addr:" << item.adr << "\n";
            ftable_stream << i << " " << item.name << " int_pointer level:" << item.level << " addr:" << item.adr << "\n";
            break;

        case Objekt::string_pointer:
            std::cout << i << " " << item.name << " string_pointer level:" << item.level << " addr:" << item.adr << "\n";
            fas_stream << i << " " << item.name << " string_pointer level:" << item.level << " addr:" << item.adr << "\n";
            ftable_stream << i << " " << item.name << " string_pointer level:" << item.level << " addr:" << item.adr << "\n";
            break;
            
        case Objekt::function:
            std::cout << i << " " << item.name << " function level:" << item.level << " size:" << item.size << "\n";
            fas_stream << i << " " << item.name << " function level:" << item.level << " size:" << item.size << "\n";
            ftable_stream << i << " " << item.name << " function level:" << item.level << " size:" << item.size << "\n";
            break;
        }
    }
